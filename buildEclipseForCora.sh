...
if [ ! $USER ]; then
	...
	echo you must specify the userid to be used when building eclipse201906forcora1, use: id -u youruserid 
elif [ ! $DOCKERGROUPID ]; then
	echo you must specify the dockergroupid to be used when building eclipse201906forcora1, use: getent group docker 
else
	...
	docker build --build-arg user=$USER --build-arg userid=$USERID --build-arg dockergroupid=$DOCKERGROUPID -t eclipse201906forcora1 cora-eclipse/docker/
