...
if [ ! $USER ]; then
  	echo you must specify the userName to be used when building eclipse201906forcora2
elif [ ! $USERID ]; then
	echo you must specify the userid to be used when building eclipse201906forcora2, use: id -u youruserid 
elif [ ! $DOCKERGROUPID ] && [ ! -d ./eclipseForCora ]; then
	echo you must specify the dockergroupid to be used when building eclipse201906forcora2, use: getent group docker 
else
	if [ ! -d ./eclipse201906forcora2 ]; then
...
#	./eclipseForCora/startEclipseForCora.sh $USER
	./eclipse201906forcora2/startEclipseForCoraTempSetup.sh $USER $ECLIPSEBRANCH
fi