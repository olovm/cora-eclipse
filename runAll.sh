...
if [ ! $USER ]; then
  	echo you must specify the userName to be used when building eclipse202412forcora3
elif [ ! $USERID ]; then
	echo you must specify the userid to be used when building eclipse202412forcora3, use: id -u youruserid 
elif [ ! $DOCKERGROUPID ] && [ ! -d ./eclipseForCora ]; then
	echo you must specify the dockergroupid to be used when building eclipse202412forcora3, use: getent group docker 
else
	if [ ! -d ./eclipse202412forcora3 ]; then
	...
	./eclipse202412forcora3/startEclipseForCoraTempSetup.sh $USER $ECLIPSEBRANCH
fi