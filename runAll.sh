...
if [ ! $USER ]; then
  	echo you must specify the userName to be used when building eclipse202412forcora2
elif [ ! $USERID ]; then
	echo you must specify the userid to be used when building eclipse202412forcora2, use: id -u youruserid 
elif [ ! $DOCKERGROUPID ] && [ ! -d ./eclipseForCora ]; then
	echo you must specify the dockergroupid to be used when building eclipse202412forcora2, use: getent group docker 
else
	if [ ! -d ./eclipse202412forcora2 ]; then
	...
	./eclipse202412forcora2/startEclipseForCoraTempSetup.sh $USER $ECLIPSEBRANCH
fi