...
if [ ! $USER ]; then
  	echo you must specify the userName to be used when building eclipse202412forcora1
elif [ ! $USERID ]; then
	echo you must specify the userid to be used when building eclipse202412forcora1, use: id -u youruserid 
elif [ ! $DOCKERGROUPID ] && [ ! -d ./eclipseForCora ]; then
	echo you must specify the dockergroupid to be used when building eclipse202412forcora1, use: getent group docker 
else
	if [ ! -d ./eclipse202412forcora1 ]; then
...
#	./eclipseForCora/startEclipseForCora.sh $USER
	./eclipse202412forcora1/startEclipseForCoraTempSetup.sh $USER $ECLIPSEBRANCH
fi