...
if [ ! $USER ]; then
  	echo "You must specify the userName used when starting eclipse201909forcora1TempSetup"
else
cd eclipse201909forcora1
docker run --rm -ti --privileged --ipc=host --env="QT_X11_NO_MITSHM=1"  -e DISPLAY=$DISPLAY \
 ...
 --name eclipse201909forcora1TempSetup\
 eclipse201909forcora1
 cd ../
fi
