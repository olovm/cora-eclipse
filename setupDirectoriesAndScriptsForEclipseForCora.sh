...
INSTALLDIR=$PARENTDIR/eclipse202603forcora2
...
createArchiveReadableFile(){
	...
	echo "docker exec -u 0 eclipse202603forcora2 bindfs --map=root/$USER:@root/@$USER /tmp/sharedArchive/ /tmp/sharedArchiveReadable/" > $PARENTDIR/archiveReadable.sh
...
createSharedFileStorageReadableFile(){
	...
	echo "docker exec -u 0 eclipse202603forcora2 bindfs --map=root/$USER:@root/@$USER /tmp/sharedFileStorage/ /tmp/sharedFileStorage/" > $PARENTDIR/sharedFileStorage.sh
