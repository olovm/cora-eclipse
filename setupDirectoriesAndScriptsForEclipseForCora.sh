...
INSTALLDIR=$PARENTDIR/eclipse202409forcora1
...
createArchiveReadableFile(){
	...
	echo "docker exec -u 0 eclipse202409forcora1 bindfs -o nonempty --map=root/$USER:@root/@$USER /tmp/sharedArchive/ /tmp/sharedArchiveReadable/" > $PARENTDIR/archiveReadable.sh
...
createSharedFileStorageReadableFile(){
	...
	echo "docker exec -u 0 eclipse202409forcora1 bindfs -o nonempty --map=root/$USER:@root/@$USER /tmp/sharedFileStorage/ /tmp/sharedFileStorage/" > $PARENTDIR/sharedFileStorage.sh
